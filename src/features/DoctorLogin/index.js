...
const DoctorLogin = () => {
  ...
  const onSubmit = async (data) => {
    ...
    axios.post("https://medico-backend-production.up.railway.app/api/doctor/login",data).then((response)=>{
      localStorage.setItem("doctorId",response.data.message);
    ...
    <div className=" flex flex-row w-full h-auto">
      <div
       className="flex w-[50%] flex-col">
        ...
        <div className=" flex items-start justify-start flex-col w-full mx-16 my-10">
          ...
          className=" w-[100%]">
            <h4 className=" text-sm font-abc1 text-black font-medium">
              {t("Email")}
            </h4>
            <div className="mt-2">
              <input
            ...
            <h4 className=" text-sm font-abc1 text-black font-medium mt-6">
              {t("Password")}
            </h4>
            <div className="mt-2">
              <input
       ...
       <h5 className=' font-abc1 text-sm font-normal text-primary-color'>{t('ForgotPassword')}</h5>
            ...
            <button
              type="submit"
              className=" flex justify-center items-center w-[70%] bg-primary-color h-[56px] rounded-[4px] my-10"
            >
              <h1 className=" text-white font-abc1 text-[16px] font-bold">
                {t("Login")}
              </h1>
            </button>
            <div className=" flex justify-center items-center flex-row w-[70%]">
              <h6 className=" text-dark-gray font-abc1 font-normal text-sm">
                {t("NotAccount")}
              </h6>
              <Link
                to="/doctorForm"
                className=" text-primary-color font-abc1 font-semibold text-sm"
              >
                {t("SignUp")}
              </Link>
            </div>
      ...
      <div className="flex w-[50%] h-full relative">
        <img
          src="./doctor.svg"
          alt="doctor"
          className=" container absolute bg-cover inset-0"
        />
        <div className="absolute inset-0 flex justify-between h-screen flex-col">
          ...
          <div className="flex justify-center items-center h-[50%]">
            <div className=" h-[50%] flex justify-center items-center bottom-0 flex-col">
              <h1 className=" font-abc1 text-[#000] font-bold text-[22px]">
                {t("ConnectDevice")}
              </h1>
              <p className=" font-abc1 text-[#000] text-[16px] font-medium">
                {t("EveryThing")}
              </p>
            </div>
          </div>
